...
- User chooses one restoration style before upload:
  - Natural Enhance: default for modern blurry phone photos; improves clarity without recreating faces.
  - Memory Restore: default for old album photos with scratches, fading, stains, or color damage.
...
- Natural Enhance uses a short, conservative prompt so modern blurry phone photos stay close to the source.
- Memory Restore always uses high input fidelity so faces and family identity stay close to the source.
...
- App creates a job with Launch Offer pricing and Natural Enhance style.
